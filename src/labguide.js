...
const Labguide = () => {
  ...
  useEffect(() => {
    // Load all symptoms for typeahead options
    const allSymptoms = ["anxiety and nervousness",
"depression",
"shortness of breath",
...
;
    setOptions(allSymptoms);
  }, []);

  const handleInputChange = (selected) => {
    // Extract labels from selected options
    setSelectedSymptoms(selected.map(item => item.label));
  };
  ...
  const updateSymptomsInput = () => {
    setSymptomsInput(selectedSymptoms.join(', '));
  };

  return (
    <div className="App">
       <h1 className="disease-predictor-title">Simple Disease Predictor:</h1>
    ...
    <div className="labguide-container">
      <form onSubmit={handleSubmit}>
        <label>Choose symptoms: (either start typing or simply select from the options)</label>
        <Typeahead
          id="symptoms-input"
          multiple
          options={options.map(symptom => ({ label: symptom }))}
          onChange={handleInputChange}
          placeholder="Select symptoms..."
        />
        <br />
        <input type="submit" value="Predict" className="predict-button" onClick={updateSymptomsInput} />
      </form>

      <div className="selected-symptoms">
       <b> <h2>Selected Symptoms:</h2></b>
        <p>{symptomsInput}</p>
      </div>
